import { Layout } from 'react-grid-layout';

export const lgLayout: Layout[] = [
    { i: 'description', x: 0, y: 0, w: 2, h: 1 },
    { i: 'spotify', x: 2, y: 0, w: 1, h: 1 },
    { i: 'theme', x: 3, y: 0, w: 1, h: 1 },

    { i: 'lbVt', x: 0, y: 1, w: 2, h: 1 },
    { i: 'lbVtSe', x: 0, y: 2, w: 2, h: 1 },
    { i: 'laifen1111', x: 2, y: 2, w: 1, h: 2 },
    { i: 'laifenSe', x: 3, y: 2, w: 1, h: 2 },

    { i: 'laifenFittings', x: 0, y: 4, w: 1, h: 2 },
    { i: 'laifenDunhuang', x: 1, y: 4, w: 1, h: 2 },
    { i: 'laifen618', x: 3, y: 4, w: 2, h: 1 },
    { i: 'laifenAbs', x: 3, y: 5, w: 2, h: 1 },


    { i: 'laifenMother', x: 0, y: 6, w: 2, h: 1 },
    { i: 'LaifenChristmas', x: 3, y: 6, w: 2, h: 1 },
    { i: 'article', x: 0, y: 7, w: 2, h: 1 },
    { i: 'contact', x: 2, y: 7, w: 2, h: 2 },



];

export const mdLayout: Layout[] = [
    { i: 'description', x: 0, y: 0, w: 2, h: 1 },
    { i: 'spotify', x: 2, y: 0, w: 1, h: 1 },
    { i: 'theme', x: 3, y: 0, w: 1, h: 1 },

    { i: 'lbVt', x: 0, y: 1, w: 2, h: 1 },
    { i: 'lbVtSe', x: 0, y: 2, w: 2, h: 1 },
    { i: 'laifen1111', x: 2, y: 2, w: 1, h: 2 },
    { i: 'laifenSe', x: 3, y: 2, w: 1, h: 2 },

    { i: 'laifenFittings', x: 0, y: 4, w: 1, h: 2 },
    { i: 'laifenDunhuang', x: 1, y: 4, w: 1, h: 2 },
    { i: 'laifen618', x: 2, y: 4, w: 2, h: 1 },
    { i: 'laifenAbs', x: 2, y: 5, w: 2, h: 1 },

    { i: 'laifenMother', x: 0, y: 6, w: 2, h: 1 },
    { i: 'LaifenChristmas', x: 2, y: 6, w: 2, h: 1 },
    { i: 'article', x: 0, y: 7, w: 2, h: 1 },
    { i: 'contact', x: 2, y: 7, w: 2, h: 1 },
];

export const smLayout: Layout[] = [
    { i: 'description', x: 0, y: 0, w: 2, h: 2 },
    { i: 'spotify', x: 0, y: 1, w: 2, h: 1},
    { i: 'theme', x: 1, y: 1, w: 2, h: 1 },

    { i: 'lbVt', x: 0, y: 2, w: 2, h: 1 },
    { i: 'lbVtSe', x: 0, y: 3, w: 2, h: 1 },
    { i: 'laifen1111', x: 0, y: 4, w: 1, h: 2 },
    { i: 'laifenSe', x: 1, y: 4, w: 1, h: 2 },

    { i: 'laifenFittings', x: 0, y: 6, w: 1, h: 2 },
    { i: 'laifenDunhuang', x: 1, y: 6, w: 1, h: 2 },
    { i: 'laifen618', x: 0, y: 8, w: 2, h: 1 },
    { i: 'laifenAbs', x: 0, y: 9, w: 2, h: 1 },

    { i: 'laifenMother', x: 0, y: 10, w: 2, h: 1 },
    { i: 'LaifenChristmas', x: 0, y: 11, w: 2, h: 1 },
    { i: 'article', x: 0, y: 12, w: 2, h: 2 },
    { i: 'contact', x: 0, y: 13, w: 2, h: 2 },
];
